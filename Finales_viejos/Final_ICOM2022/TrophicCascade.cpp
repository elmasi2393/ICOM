...
struct Pasto
{
    ...
    void crecer(){ if (disponibilidad<pastizal_maximo) disponibilidad+=rate_pasto;}
...
struct Presa : public Animal
{
    ...
    Presa(bool sexo, unsigned int t_mapa);
	
	//la presa aparece en una ubicacion determinada (TODO)
    Presa(bool sexo, Punto2D p);
...
struct Depredador : public Animal
{
    ...
    Depredador(bool sexo, unsigned int t_mapa);
    
    //el depredador aparece en una ubicacion determinada (TODO)
    Depredador(bool sexo, Punto2D p);
...
private:
    ...
    Pasto * get_pasto(const Punto2D &p);
